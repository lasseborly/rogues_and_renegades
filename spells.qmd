...
The number of spells of each level which a Cleric or Wizard may cast per day is shown on the appropriate table in the [Classes](class.qmd#character-classes) section. Each day, usually in the morning, spellcasters prepare spells to replace those they have used. Clerics do this through prayer, while Wizards must study their spellbooks. Spells prepared but not used persist from day to day; only those actually cast must be replaced. A spellcaster may always choose to dismiss a prepared spell (without casting it) in order to prepare a different spell of that level.
...
Wizards cast spells through the exercise of knowledge and will. They prepare spells by study of their spellbooks; each Wizard has his or her own spellbook containing the magical formulae for each spell the Wizard has learned. Spellbooks are written in a magical script that can only be read by the one who wrote it, or through the use of the spell **read magic**. All Wizards begin play knowing **read magic**, and it is so ingrained that it can be prepared without a spellbook.

A Wizard may only prepare spells after resting (i.e. a good night's sleep), and needs one turn per each three spell levels to do so (rounding fractions up). Spells prepared but not used on a previous day are not lost. For example, a 3^rd^ level Wizard preparing all three of his or her available spells (two 1^st^ level and one 2^nd^ level) is preparing a total of 4 levels of spells, and thus needs 2 turns (4 divided by 3 and rounded up).
...
When cast upon any magical text, such as a spellbook or wizard spell scroll, this spell enables the caster to read that text. Casting this spell on a cursed text will generally trigger the curse. All Wizards begin play knowing this spell, and it can be prepared even if the Wizard loses access to his or her spellbook.
...
A **wizard lock** spell cast upon a door, chest, or portal magically locks it. The caster can freely pass his or her own wizard lock without affecting it, as can any Wizard 3 or more levels higher than the caster of the wizard lock; otherwise, a door or object secured with this spell can be opened only by breaking in or with a successful **dispel magic** or **knock** spell.
