...
Normal characters have a chance equal to a roll of 1 on 1d6 to detect a trap if a search for one is made. Note that this is about a 16.7% chance. Search for traps takes at least a turn per 3 meter square area. A single character may only effectively search a given area for traps once.

Trap detection may not be allowed if the trap is purely magical in nature; on the other hand, in such cases Wizards and/or Clerics may be able to detect magical traps at the given 1 in 1d6 chance, at the Game Master's option.
