...
For example: Darion the Human Fighter and Morningstar the Elven Fighter/Wizard open a door and come face-to-face with a party of goblins. The GM rolls 1d6 for the goblins; on a 1-2 they are all surprised. Then the GM rolls 1d6 for Darion and Morningstar. If the roll is a 1, both of them are surprised; if the roll is a 2, only Darion is surprised. If the roll is 3 or more, neither of them are surprised.
...
| Fighter Level | Cleric level | Wizard Level | Monster Hit Dice | Attack Bonus |
|---|---|----|----|----|
...
Attacks made from behind an opponent usually receive a +2 attack bonus. This does not combine with the [Sneak Attack](class.qmd#thievery-abilities) ability.
...
If a character attempts to use a missile weapon against a foe who is within 5' of him or her (i.e. who is engaged with the shooter), the attack roll is made with disadvantage. This is due to the shooter dodging around to avoid the foe's attacks. The only exception is if the attacker is behind the target creature and undetected, or that creature is distracted so as to not be able to attack the shooter; in these cases, apply the usual +1 bonus (+3 total bonus if attacking from behind).
...
: Wizard Saving Throws {#tbl-save-mage-2}
